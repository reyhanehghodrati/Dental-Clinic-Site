...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (!empty($_POST['day']) && !empty($_POST['time'])) {
        ...
        $check_sql = "SELECT * FROM dbo_schedule_nobat WHERE day_of_week = '$day' AND time_slot = '$time'";
        $result = $conn->query($check_sql);

        if ($result->num_rows > 0) {
            $_SESSION['message'] = "<span style='color: red;'>این بازه زمانی قبلاً ثبت شده است!</span>";
        } else {
            $sql = "INSERT INTO dbo_schedule_nobat (day_of_week, time_slot) VALUES ('$day', '$time')";
