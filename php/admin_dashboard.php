...
$health_test_sql = "SELECT * FROM health_questions_responses"; // جدول مربوط به آزمون سلامت دندان
$consultation_sql = "SELECT * FROM contact_messages"; // جدول مربوط به مشاوره آنلاین
$doctors = "SELECT * FROM reservation_doctor_profiles"; // جدول مربوط به نوبت‌دهی
$slider_settings = 'SELECT * FROM slider_details ORDER BY priority ASC';
$comments_sql='select * from user_feedback';
$nobat='select * from reservation_schedule_slots order by day_of_week';
$time_reservation='SELECT * FROM `reservation_requests` ORDER BY `tarikh` , `doctor_id` ';
...
$sql = "SELECT * FROM slider_details LIMIT 1";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $max_priority_query = "SELECT MAX(priority) AS max_priority FROM slider_details";
    ...
    $insert_sql = "INSERT INTO slider_details (title, subtitle, background_image, priority, created_in) VALUES (?, ?, ?, ?, NOW())";
        ...
        <table>
            ...
            <?php while ($row = mysqli_fetch_assoc($slider_settings_result)) { ?>
                <tr>
                    ...
                    <td>
                        <a href="slider_delete.php?id=<?= $row["id"] ?>& background_image=<?= $row["background_image"] ?> "methods="get" onclick="return confirm('ایا مطمین هستید')" >حذف     </a>
                        <p>------</p>
                        <a href="slider_update.php?id=<?= $row["id"] ?> "methods="get">ویرایش     </a>
                        <p>------</p>
                        <a href="slider_update_priority.php?id=<?= $row["id"] ?>&direction=up">⬆ بالا</a>
                        <a href="slider_update_priority.php?id=<?= $row["id"] ?>&direction=down">⬇ پایین</a>
                    </td>
                    <td>
                        <?php
                        if ($row['last_updated']) {
...
<section>
        ...
        <table>
            ...
            <?php while ($row = mysqli_fetch_assoc($comments_result)) { ?>
                <tr>
                    ...
                    <td><a href="comments_delete.php?id=<?= $row["id"] ?> "methods="get" onclick="return confirm('ایا مطمین هستید')" >حذف     </a>
                    </td>
        ...
        <table>
            ...
            <?php while ($row = mysqli_fetch_assoc($time_reservation_result)) { ?>
                <tr>
                    ...
                    <td><a href="reservation_nobat_delete.php.php?id=<?= $row['id'] ?>" onclick="return confirm('آیا مطمئن هستید؟')">حذف</a>
                        <p>------</p>
                        <a href="reservation_nobat_update.php
                        ?id=<?= $row["id"] ?> "methods="get">ویرایش     </a></td>
                </tr>
            <?php } ?>
            </tbody>
        </table>
    ...
    <div class="table-container">
        ...
        <form method="post" action="reservation_doctor_schedule.php" enctype="multipart/form-data">
            <label>نام پزشک:</label>
            <select name="doctor_id">
                <?php
                $sql='select * from reservation_doctor_profiles';
            ...
            <select name="schedule_id">
                ...
                $sql='select * from reservation_schedule_slots ';
                $result=$conn->query($sql);
                while ($row = $result->fetch_assoc()) {
        ...
        <table>
            ...
            $sql = "SELECT ds.id as doctor_schedule_id, d.id as doctor_id, d.name, d.takhasos, s.id as schedule_id, s.day_of_week, s.time_slot 
                ...
                JOIN reservation_doctor_profiles d ON ds.doctor_id = d.id
            ...
            while ($row = $result->fetch_assoc()) { ?>
                <tr>
                    ...
                    <td><a href="reservation_doctor_schedule_delete.php?id=<?= $row['doctor_schedule_id'] ?>" onclick="return confirm('آیا مطمئن هستید؟')">حذف</a></td>                    </tr>
            <?php } ?>
            </tbody>
        </table>
    </div>
    ...
    <div class="table-container">
        <h3>نوبت کاری </h3>
        <form method="post" action="reservation_schedule.php" enctype="multipart/form-data">
            <label>روز:</label>
            <select name="day">
        ...
        <table>
            ...
            <?php while ($row = mysqli_fetch_assoc($nobat_result)) { ?>
                <tr>
                    <td><?php echo $row['day_of_week']; ?></td>
                    <td><?php echo $row['time_slot']; ?></td>
                    <td><a href="reservation_schedule_delete.php?id=<?= $row["id"] ?> "methods="get" onclick="return confirm('ایا مطمین هستید')" >حذف     </a></td>
                </tr>
            <?php } ?>
            </tbody>
        </table>
